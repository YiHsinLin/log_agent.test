...
proto.listen = function(port, cb) {
    ...
    var parserFn = this.parse || parser;
...
function parser(msg, rinfo) {
    ...
    var timeStr = format.substring(priIndex + 1, last2SpaceIndex).trim();
    var time = moment(timeStr, 'YYYY-MM-DD hh:mm:ss.SSS');
    if (!time.isValid()) {
